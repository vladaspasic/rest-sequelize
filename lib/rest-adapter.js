...
var CoreObject = require('./core'),
	...
	DefaultResolver = require('./default-resolver');
...
var RestAdapter  = CoreObject.extend({
	...
	init: function() {
		...
		var Resolver = this.Resolver || DefaultResolver;
	...
	serviceFor: function(type) {
		...
		var cacheKey = 'service:' + type;

		var service = this._cache[cacheKey];

		if(!service) {
			service = this.resolver.resolveService(type);

			if(!service) {
				throw new Error('Could not resolve service for type `' +  type + '`.');
			}

			this._cache[cacheKey] = service;
		}
	...
	serialize: function(result, meta, status) {
		...
		if (_.isArray(result)) {
			serialized = _.map(result, serializeRecord);
		} else {
			serialized = serializeRecord(result);
		}
